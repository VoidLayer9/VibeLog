...
| `navbar`      | boolean | If `true`, this category appears as a button in the navbar |
| `description` | string  | Category description (available for tooltips or meta info) |

> **Note:** To add or remove categories from the navbar, simply edit `categorys.json` and set the `navbar` field to `true` or `false`. No code changes required.
