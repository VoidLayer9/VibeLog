...
| Root password   | Configurable via argv flag `--root-password` (stored in global constant) |
| Database path   | Configurable via argv flag `--database-path` |
...
### 2.7 Management API (DevOps)

These routes are used for database management and synchronization.

**Authentication:** All management routes require the `root-password` header to match the server's configured root password.

#### `POST /api/write_database_file` — Write File

**Headers:**
- `path`: Relative path of the file to write (inside database directory).
- `root-password`: The server's root password.

**Body:** Raw file content.

#### `POST /api/read_database_file` — Read File

**Headers:**
- `path`: Relative path of the file to read.
- `root-password`: The server's root password.

**Response:** The raw content of the requested file.

#### `POST /api/delete_database_file` — Delete File
...
#### `POST /api/list_database_files_recursively` — List Files

**Headers:**
- `path`: (Optional) Directory to list. Defaults to root.
- `root-password`: The server's root password.
...
The application should support the following CLI commands for management and synchronization.

### 6.1 Start Server

```bash
vibelog start --port 8080 --database-path /path/to/database --root-password your_password
```

- Starts the HTTP server on the specified port.
- Uses the provided database path.
- Sets the global root password for API access.

### 6.2 Upload Database (Sync Up)

```bash
vibelog upload --path /path/to/file --root-password your_password --url http://localhost:8080
```

- Uploads a local file to the server using `/api/write_database_file`.
- Authenticates using the root password.

### 6.3 Download Database (Sync Down)

```bash
vibelog download --path /path/to/file --root-password your_password --url http://localhost:8080
```

- Downloads a file from the server using `/api/read_database_file`.
- Authenticates using the root password.