...
| Root password   | Configurable via argv flag `--root_password` (stored in global constant) |
| Database path   | Configurable via argv flag `--database_path` |
...
**Authentication:** All management routes require the `root_password` header to match the server's configured root password.

#### `POST /api/write_database_file` — Write File

**Headers:**
- `path`: Relative path of the file to write (inside database directory).
- `root_password`: The server's root password.
