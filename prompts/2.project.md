...
**VibeLog** is a static-dynamic blog called **mateusmoutinho**, served by an HTTP server written in pure C (editing `app.c`). The database is file-based on the filesystem (no SQL).

### 1.1 Visual Identity

| Aspect          | Definition                                                                 |
|-----------------|---------------------------------------------------------------------------|
| Theme           | Dark, minimalist, with "demonic" / occultist atmosphere                   |
| Primary palette | Background: `#0a0a0a`, Cards: `#111111`, Borders: `#1a1a1a`              |
| Accent palette  | Blood red: `#8b0000`, Hover red: `#a00000`, Runic orange: `#cc4400`      |
| Text            | Primary: `#e0e0e0`, Secondary: `#888888`, Links: `#cc4400`               |
| Typography      | Titles: serif (e.g. `Cinzel`, `Playfair Display`), Body: sans-serif (e.g. `Inter`, `Source Sans Pro`) |
| Icons           | Minimalist, monochromatic, no emojis                                      |
| References      | Inspiration: sites like [motherfuckingwebsite.com](http://motherfuckingwebsite.com) (minimalism) + medieval grimoire aesthetics |
...
|-------------------------------|----------------------------------------------------------------------|
| Logo/Blog name                | `/`                                                                  |
| Category buttons (dynamic)    | Generated from `database/navbar_categorys.json` — each entry produces a button linking to `/list_articles?page=1&category={name}&limit=10` |
| Search field                  | On submit, redirects to `/list_articles?page=1&limit=10&search={input}` |

**Category source file:** `database/navbar_categorys.json`

```json
[
    {
        "name": "Lua",
        "description": "Artigos sobre a linguagem Lua e seu ecossistema"
    },
    {
        "name": "C",
        "description": "Desenvolvimento de baixo nível e arquitetura em C"
    },
    {
        "name": "Vibecoding",
        "description": "Filosofia de código, minimalismo e estética"
    }
]
```
...
|---------------|--------|----------------------------------------------------------|
| `name`        | string | Category name displayed on the button and used as the `category` query param |
| `description` | string | Category description (available for tooltips or meta info) |

> **Note:** To add or remove categories from the navbar, simply edit `navbar_categorys.json`. No code changes required.
